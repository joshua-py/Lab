...
void addsparse(int b1[MAX][3],int b2[MAX][3],int b3[MAX][3])
{
    ...
    {
        printf("nYou have entered invalid matrix!!Size must beequal");
        exit(0);
    }
    ...
    while(i<=t1&&j<=t2)
    {
        ...
        {
            b3[k][0]=b1[i][0];      //row andcolumn numbers are equal
            b3[k][1]=b1[i][1];
    ...
    while(i<=t1)        //copyremaining terms from b1
    {
        b3[k][0]=b1[i][0];
        b3[k][1]=b1[i][1];
        b3[k][2]=b1[i][2];
    ...
    while(j<=t2)        //copyremaining terms from b2
    {
        b3[k][0]=b2[j][0];
        b3[k][1]=b1[j][1];
        b3[k][2]=b1[j][2];
